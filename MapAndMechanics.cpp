...
void readMap(char(*map)[10], Enemy ** enemy, int numEnemies)
{
	...
	for (int i = 0; i < numEnemies; i++)
	{
		...
		//RESEARCH OF DYNAMIC_CAST GOES HERE
		if (Diablo* enemyCast = dynamic_cast<Diablo*>(enemy[i]))
		{
			map[enemiesX[i]][enemiesY[i]] = 'D';
		}
		else if (Skeleton* enemyCast = dynamic_cast<Skeleton*>(enemy[i]))
		{
			map[enemiesX[i]][enemiesY[i]] = 'S';
		}
		else if (Heretic* enemyCast = dynamic_cast<Heretic*>(enemy[i]))
		{
			map[enemiesX[i]][enemiesY[i]] = 'H';
		}
		else if (Necromancer* enemyCast = dynamic_cast<Necromancer*>(enemy[i]))
		{
			map[enemiesX[i]][enemiesY[i]] = 'N';
		}
	}
}
...
Enemy** setDifficulty(String& difficulty, int& numEnemies)
{
	...
	else if (difficulty == "Medium")
	{
		...
		mediumEnemies[4]->setXEnemy(8); mediumEnemies[4]->setYEnemy(9);
